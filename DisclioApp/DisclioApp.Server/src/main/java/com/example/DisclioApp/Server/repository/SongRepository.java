...
public interface SongRepository extends JpaRepository<Song, Integer> {
    ...
    // Requirement: Relational Filter
    @Query("SELECT s FROM Song s WHERE s.cd.id = :cdId")
    List<Song> findByCdId(@Param("cdId") int cdId);

    // Requirement: CRUD (Delete related entities)
    @Modifying
    @Transactional
    @Query("DELETE FROM Song s WHERE s.cd.id = :cdId")
    void deleteByCdId(@Param("cdId") int cdId);
}
